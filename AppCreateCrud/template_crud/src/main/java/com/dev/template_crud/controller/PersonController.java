...
public class PersonController {
    ...
    private boolean isAnyFieldNullOrEmpty(Person person) {
    	if (person.getFirstname() == null || person.getFirstname().trim().isEmpty() || person.getLastname() == null || person.getLastname().trim().isEmpty() || person.getCin() == null || person.getCin().trim().isEmpty() || person.getBithday() == null) {
    		return true;
		} else {
			return false;
		}
...
@GetMapping("/detail/region/update/{person_id}/{region_id}")
	...
	public String updateRegionInDetailPerson(@ModelAttribute("region") Region region,@RequestParam("region_id") Integer region_id, @RequestParam("person_id") Integer person_id) {
		try {
			Person person = this.personService.detail(person_id);
	...
	public String deletePersonInDetail(@PathVariable("person_id") Integer person_id, @PathVariable("id") Integer theId) {
		try {
